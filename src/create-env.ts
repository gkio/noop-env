...
export function createEnv<TInput, TOutput extends object, TGroupedOutput = Group<TOutput>>(
...
function handleValidationErrors(errors: z.ZodError['errors']): never {
  ...
  throw new ValidationError(`Validation of the following environment variables failed: "${affectedVariables.join('", "')}"`, fieldErrors);
}
...
function generateErrorMessage(fieldName: string, error: z.ZodIssue): string {
  ...
  switch (error.code) {
    case 'invalid_type':
      errorMessage = error.received === 'undefined'
    ...
    case 'invalid_enum_value':
      errorMessage = `Enum mismatch: The variable "${fieldName}" must be one of the following values: "${error.options.join('", "')}", but received "${error.received}"`;
      break;
    case 'invalid_string':
...
function generateStringErrorMessage(fieldName: string, error: z.ZodInvalidStringIssue): string {
  ...
  return `Format error: The value of variable "${fieldName}" must include "${error.validation.includes}"${error.validation.position ? ` at position ${error.validation.position}` : ''}`;
}
